...
namespace VerusDate.Web.Api
{
    public static class ProfileApi
    {
        ...
        public async static Task<Profile> Profile_Get(this HttpClient http, ILocalStorageService storage)
        {
            if (string.IsNullOrEmpty(StorageKey)) return null;

            if (!await storage.ContainKeyAsync(StorageKey))
            {
                await storage.SetItemAsync(StorageKey, await http.GetCustom<Profile>("Profile/Get"));
            }

            return await storage.GetItemAsync<Profile>(StorageKey);
        }
        ...
        public async static Task<Profile> Profile_GetView(this HttpClient http, string IdUserView)
        {
            ...
            return await http.GetCustom<Profile>($"Profile/GetView/{IdUserView}");
        }
        ...
        public async static Task<List<Profile>> Profile_ListMatch(this HttpClient http)
        {
            return await http.ListCustom<Profile>("Profile/ListMatch");
        }

        public async static Task<List<Profile>> Profile_ListSearch(this HttpClient http)
        {
